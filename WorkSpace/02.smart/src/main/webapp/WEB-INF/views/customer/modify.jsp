...
<body>
	...
	<input type="hidden" name="id" value="${vo.id }">
				...
				<div class="form-check form-check-inline">
  					<input class="form-check-input" type="radio" name="gender" id="gender" value="남"
  					 ${vo.gender eq "남" ? "checked" : "" }>
  						<label class="form-check-label" for="gender"> <!-- label의 for과 input의 id가 동일해야함 -->
    						남
  						</label>
					</div>
				<div class="form-check form-check-inline">
		...
		<div class="btn-toolbar gap-2 my-3 justify-content-center">
			...
			<button type="button" class="btn btn-outline-danger" onclick="history.go(-1)">취소</button> <!-- 뒤로가기랑 같은 방식 -->
			<%-- <button type="button" class="btn btn-outline-danger" onclick="location='info.cu?id=${vo.id}'">취소</button> --%>
		</div>
	</form>
</body>
</html>